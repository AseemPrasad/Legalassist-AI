...
def _get_otp_rate_limit_script():
    ...
    In the shim, we still expose the hook so tests can stub it, but the actual
    enforcement below is backed by the database and an in-process timestamp
    window so OTP throttling works even without Redis.
    """

    def _script(*, keys, args):
        # Default shim behavior is intentionally inert: the real limiter below
        # owns enforcement, while tests can monkeypatch this hook to observe keys.
        return 0
...
def _reserve_otp_rate_limit_slot(
    ...
    The shim enforces the limit locally so OTP throttling still works when
    Redis is unavailable. The compatibility script hook is still invoked so
    existing tests and monkeypatches can observe the reserved keys.
    """
